...
namespace App\Feature\Timeline;
...
class TimelineRepository
{
    ...
    public function getPost(int $id_post): ?Timeline
    {
        $stmt = $this->connection->getConnection()->prepare(
            "SELECT p.id AS post_id,
                    p.post AS message,
                    CASE
                        WHEN TIMEDIFF(NOW(), p.date) < '01:00:00' THEN
                            CONCAT('il y a ', TIME_FORMAT(TIMEDIFF(NOW(), p.date), '%imin'))
                        WHEN TIMEDIFF(NOW(), p.date) < '24:00:00' THEN
                            CONCAT('il y a ', TIME_FORMAT(TIMEDIFF(NOW(), p.date), '%kh et %imin'))
                        WHEN TIMEDIFF(NOW(), p.date) < '30 00:00:00' THEN
                            CONCAT('il y a ', TIMESTAMPDIFF(DAY, p.date, NOW()), ' jours')
                        ELSE
                            DATE_FORMAT(p.date, '%d/%m/%Y')
        ...
        if ($result) {
            ...
            $timeline->id = $result['post_id'];
            $timeline->message = $result['message'];
            $timeline->count_likes = $result['nb_likes'];
            $timeline->count_retweets = $result['nb_retweets'];
            $timeline->count_comments = $result['nb_comments'];
            $timeline->date_in_minutes = $result['time_difference'];
            $timeline->author = $result['author'];
            $timeline->author_photo = $result['author_photo'];
