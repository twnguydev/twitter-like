...
use App\Feature\Timeline\TimelineRepository;
...
class TimelineController
{
    ...
    public function setPostRegistration()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($_POST['message']) && !empty($_POST['message'])) {
                ...
                if (empty($errors)) {
                    $timelineRep = new TimelineRepository();

                    $setPost = $timelineRep->setPostRegistration($userRep->isUserLogged()->id, $message, $hashtags, $images);
    ...
    public function getPostPhoto($params)
    {
        ...
        $app = new ConfigApp();
        $timelineRep = new TimelineRepository();
        $image_path = $timelineRep->getImageFromHash($image_hash);
    ...
    public function getPostView($params)
    {
        ...
        $timelineRep = new TimelineRepository();
        $post = $timelineRep->getPost($id_post);
    ...
    private function replaceAllHashtags($message)
    {
    ...
    private function replaceAllArobases($message)
    {
        if (preg_match_all('/\s@(\w+)/', $message, $matches)) {
            foreach ($matches[1] as $match) {
    ...
    private function replaceAllLinks($message)
    {
    ...
    private function generateShortUrl($file_name)
    {
        $hash = md5($file_name);
