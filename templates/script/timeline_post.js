$(document).ready(function () {
    ...
    $("#post-add-photos").on("click", function () {
        let input = $("<input type='file' accept='image/*' multiple>");
            ...
            if (files.length > 0) {
                ...
                for (let i = 0; i < files.length; i++) {
                    if (files[i].size <= maxSize) {
                        ...
                        alert("La taille de l'image " + files[i].name + " dépasse la limite autorisée.");
                    }
                }
                ...
                .done(function(response) {
                    ...
                    if (response.success && response.links) {
                        ...
                        let currentContent = $("#post-textarea").val();
                        let newContent = currentContent + '\n\n' + response.links.join('\n');
                        $("#post-textarea").val(newContent);
                    } else {
                        console.log("success:", response.success, "links:", response.links);
                    }
                })                               
                .fail(function(jqXHR, textStatus, errorThrown) {
                    console.error("Échec de la requête AJAX:", textStatus, errorThrown);
                });                
            }
